...
/// a VERY SPECIAL Purpose built custom vector type JUST
/// for sqlite3. it guarentees the following properties:
/// 1: it ALWAYS allocates one 'block' where
/// each memory allocation is guarenteed to be a multiple
/// of __m256i
/// 2: the allocation is always allocated against the zero page
/// such that any non bounded memory reads always read
/// or get compared against zero (hopefully without crashing)
/// 3: the logical length of the data is stored alongside
/// the logical amount of blocks stored
/// 4: any references to this structure remain valid
/// EVEN IF the underlying allocations are moved,removed,resized or otherwise
/// UNLESS the container type gets dropped
/// 5: the underlying pointer types and references
/// use NonNull and NonZero<T> when it makes sense
/// for llvm optimizations
/// NOTE: this data type will only EVER
/// contain implementations that round exactly to multiples
// of __m256i
