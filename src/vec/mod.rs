use std::{alloc::{Layout, alloc_zeroed, dealloc}, fmt::{Binary, Display}, num::NonZeroUsize, ops::{Index, IndexMut}, ptr::NonNull};

use crate::mem::Sqlite3Memory;

#[cfg(target_arch="x86_64")]
pub mod x86_64;

pub struct Sqlite3Vec<T> {
...
impl<T> Sqlite3Vec<T> {
    ...
    /// --- The Type-Level Enforcer ---
    /// This unnamed constant forces the compiler to evaluate the boolean expression
    /// when Sqlite3Vec<T> is instantiated. If the type is zero-sized, the calculation
    /// underflows a usize, resulting in a strict compile-time error.
    const CHECK: () = {
        if std::mem::size_of::<T>() == 0 {
            panic!(
                "Sqlite3Vec: Zero-Sized Types (ZSTs) are explicitly outlawed from this collection!"
            );
        }
    };
    ...
    #[inline(always)]
    pub const fn size_of_inner() -> NonZeroUsize {
        // 2. Safely read the size of T knowing it is mathematically >= 1 byte.
        let size_of_t: usize = std::mem::size_of::<T>();

        unsafe {
            // Completely branchless. LLVM treats this cast as an absolute
            // geometric truth, emitting no safety checks in the final assembly.
            NonZeroUsize::new_unchecked(size_of_t)
        }
    }
}
    ...
    T: Sized + Default,
{
    ...
    pub fn new() -> Self {
        ...
        unsafe { std::ptr::write_bytes(raw_ptr, 0, layout.size()) };
        let default = T::default();
        unsafe { raw_ptr.cast::<T>().write(default) };
        // let raw_ptr = unsafe {std::intrinsics::const_make_global(raw_ptr) as *mut u8};
        let ptr = unsafe { NonNull::new_unchecked(raw_ptr).cast::<T>() };
    ...
    T: Default + Clone,
{
    pub fn with_initial_size(elements: NonZeroUsize, value: Option<T>) -> Self {
        ...
        for i in 0..elements.get() {
            unsafe { ptr.add(i).write(value.clone().unwrap_or_default()) };
        }
        Self {
            ptr,
            size,
    ...
    pub fn from_slice<B: AsRef<[T]>>(b: &B) -> Result<Self, &'static str> {
        ...
        unsafe {
            ...
            std::ptr::copy_nonoverlapping(bytes.as_ptr(), ptr as *mut T, len.get());
        }
    ...
    T: Sized,
{
    pub const fn layout(elements: NonZeroUsize) -> Layout {
        ...
        let final_allocated_bytes = if remainder == 0 {
            ...
            exact_bytes.checked_add(size_of_register.get() - remainder)
...
impl<T> std::fmt::Display for Sqlite3Vec<T>
where
    T: Display,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = unsafe { self.ptr.cast_slice(self.len().get()).as_ref() };
        let mut iter = s.iter();
        write!(f, "[")?;
...
// impl<T> std::fmt::Debug for Sqlite3Vec<T>
// where
//     T: Debug,
// {
//     fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
//             self.ptr
//                 // .cast::<u8>()
//                 .cast_slice(self.size.get() * size_of::<Sqlite3Memory>() / size_of::<T>())
//                 .as_ref()
//         };
//         write!(
//             f,
//             "Sqlite3Vec{{ len:{},ptr {:p}}}\n Data:{s:?}",
//             self.len, self.ptr
//         )
//     }
// }
    ...
    T: std::fmt::Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let slice = unsafe { self.ptr.cast_slice(self.len().get()).as_ref() };
        write!(
            f,
            "Sqlite3Vec<{}>{{ len: {}, size {}, ptr: {:p} }}\n Data: [",
            std::any::type_name::<T>(),
            self.len,
            self.size,
            self.ptr
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            for _ in 0..self.len().get() {
                std::ptr::drop_in_place(current_ptr);
...
impl<T> PartialEq for Sqlite3Vec<T> where T: PartialEq {
    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        */
        ...
        if self.size  != other.size || self.len() != other.len() {
            return false;
        }
...
impl<T> Eq for Sqlite3Vec<T> where T: Eq {}

impl<T> std::fmt::Binary for Sqlite3Vec<T>
where
    T: Binary,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    T: Sized,
{
    ...
    pub fn push(&mut self, t: T) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn pop(&mut self) -> T {
        ...
        unsafe {
            if self.len == NonZeroUsize::new_unchecked(1) {
                let last_item = std::ptr::read(self.ptr.as_ptr());
                if std::mem::needs_drop::<T>() {
                    std::ptr::drop_in_place(self.ptr.as_ptr());
                }
                std::ptr::write(self.ptr.as_ptr(), T::default());
                return last_item;
            }
        }
        unsafe {
            ...
            let popped_item = std::ptr::read(target_ptr);
            if std::mem::needs_drop::<T>() {
                std::ptr::drop_in_place(target_ptr);
            }
            std::ptr::write_bytes(target_ptr, 0, 1);
...
impl<T> Index<usize> for Sqlite3Vec<T> {
    type Output = T;
    #[inline(always)]
...
/// The full responsibility goes on the developer to ensure
/// that the index is always bounded!
impl<T> IndexMut<usize> for Sqlite3Vec<T> {
    #[inline(always)]
...
impl Sqlite3Vec<u8> {
    ...
    pub fn encode_as_wide_text_unchecked(&mut self, buf: &mut Sqlite3Vec<u16>) {
        let mut offset = 0;
        self[0] = 123;
    }
}

pub fn index_mut_with_eq_asm_output_check(lhs: &mut Sqlite3Vec<u64>, rhs: &mut Sqlite3Vec<u64>)-> bool {
    lhs[0] = 64;
    rhs[20] = 128;
    let _ = rhs.pop();
    lhs.push(256).ok();
...
#[cfg_attr(test, test)]
pub fn test_vec() {
    let mut vec: Sqlite3Vec<u8> = Sqlite3Vec::from_slice(&[0, 1, 2, 3, 4, 5, 6, 7, 8]).unwrap();
    // println!("{vec}");
    // println!("{vec:?}");
    // assert_eq!(vec, vec);
    let _ = vec == vec;
...
impl std::fmt::Binary for AwkwardCell {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
...
impl<T> Sqlite3Vec<T> {
    ...
    pub fn realloc_zeroed(&mut self, new_size: NonZeroUsize) {
        ...
        let new_ptr =
            unsafe { std::alloc::realloc(self.ptr.as_ptr().cast(), old_layout, new_layout.size()) };
        ...
        unsafe {
            ...
            std::ptr::write_bytes(new_start.as_ptr(), 0, new_count);
        }
