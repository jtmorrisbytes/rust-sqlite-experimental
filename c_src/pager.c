...
// Jordan: we did away with '0 means no page, and we guarentee'
// that if the code reaches this point, then there is at least
// one valid page block (via rust nonzerousize)
static int getPageNormal(
  Pager *pPager,      /* The pager open on the database file */
  Pgno pgno,          /* Page number to fetch (1-indexed) */
  DbPage **ppPage,    /* Write a pointer to the page here */
  int flags           /* PAGER_GET_XXX flags */
){
  // Convert 1-indexed SQLite page numbers to your 0-indexed array offsets.
  // Page 1 (Header) maps to offset 0 cleanly.
  size_t page_id = (size_t)pgno - 1;

  // 1. Fetch the raw, unaligned-free byte address directly out of your Rust core.
  // This handles the elastic virtual page commits automatically under the hood!
  unsigned char *raw_virtual_ptr = sqlite3r_pcache_fetch(page_id);

  // 2. Allocate your lightweight, 16-byte structural tracking token shell.
  // This satisfies the B-Tree's expectation without polluting CPU cache lines.
  DbPage *pPg = (DbPage*)sqlite3MallocZero(sizeof(DbPage));
  if( pPg==0 ) return SQLITE_NOMEM_BKPT; // Hard hardware wall fallback

  pPg->pBuf = (void*)raw_virtual_ptr;
  pPg->pgno = pgno;
  pPg->pPager = pPager;

  // 3. Check if the page is missing from disk bounds and needs a raw read.
  // If PAGER_GET_NOCONTENT is set or it's a completely fresh page past current dbSize,
  // we do absolutely nothing because your Rust virtual layer ALREADY zeroed the memory!
  int noContent = (flags & PAGER_GET_NOCONTENT)!=0;
  if( isOpen(pPager->fd) && pgno <= pPager->dbSize && !noContent && !pPager->aStat[PAGER_STAT_HIT] ){
    // Explicitly update the hit/miss tracker for SQLite's internal metrics
    pPager->aStat[PAGER_STAT_MISS]++;
    
    /* 
     * Call readDbPage. This triggers the native OS VFS layer to issue 
     * a direct disk read directly into your raw virtual byte address!
     */
    ...
    // Cache hit or fresh page boundary: Metrics tick upward instantly
    pPager->aStat[PAGER_STAT_HIT]++;
  }

  // Assign back to the query execution pipeline
  *ppPage = pPg;
  return SQLITE_OK;
}
#if SQLITE_MAX_MMAP_SIZE>0
