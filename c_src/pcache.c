...
**   pointers).
*/
struct PCache {                       /* Last synced page in dirty page list */
  ...
  int (*xStress)(void*);       /* Call to try make a page clean */
...
*/
// jordan here: for now, we rip out all the beautiful customization features and hard wire our pcache impl
...
int sqlite3PcacheOpen(
  ...
  if (szPage != 4096) return NULL;
    // Read the default global capacity hint
  int initial_cap = sqlite3GlobalConfig.nPage;
  ...
  sqlite3r_pcache_init_thread((size_t)initial_cap/*, (size_t)szPage*/);
  return (PCache*)(size_t)1; 
}
...
sqlite3_pcache_page *sqlite3PcacheFetch(
  ...
  pRes = sqlite3r_pcache_fetch((size_t)pgno);
  return pRes;
}
